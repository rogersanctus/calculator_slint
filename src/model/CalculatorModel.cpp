...
CalculatorModel::CalculatorModel() : m_display("0"), m_accumulator(0), m_pendingOp(0), m_newInput(true) {}

std::string CalculatorModel::display() const {
    return m_display;
}
...
void CalculatorModel::inputDigit(int d) {
    if (m_newInput) {
        ...
        if (m_display == "0") m_display = std::to_string(d);
        else m_display += std::to_string(d);
    }
}
...
void CalculatorModel::inputEquals() {
    ...
    if (!m_display.empty() && m_display.back() == '.') m_display.pop_back();
    m_newInput = true;
}
